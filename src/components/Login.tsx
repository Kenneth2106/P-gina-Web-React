...
function Login({ usuarioRegistrado, onLogin, onNavigate }: LoginProps) {
	...
	return <section className="form-page login-page"><div className="form-intro"><p className="eyebrow">Qué bueno verte</p><h1>Vuelve a tu<br /><em>próximo rumbo.</em></h1><p>Tu mapa de lugares pendientes te está esperando.</p></div><div className="form-card"><div className="form-heading"><span>01 / 02</span><h2>Iniciar sesión</h2><p>Ingresa las credenciales de tu cuenta.</p></div><form onSubmit={iniciarSesion}><label>Correo electrónico<input type="email" placeholder="tu@correo.com" value={email} onChange={(evento) => setEmail(evento.target.value)} /></label><label>Contraseña<input type="password" placeholder="Tu contraseña" value={password} onChange={(evento) => setPassword(evento.target.value)} /></label><button className="button button-dark form-submit" type="submit">Entrar a Rumbo <span>↗</span></button></form><p className="switch-form">¿Aún no tienes cuenta? <button onClick={() => onNavigate("registro")}>Regístrate</button></p></div></section>;
}
